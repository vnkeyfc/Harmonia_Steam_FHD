...
for _dir in ['HAR']:
    ...
    for imagepath in os.listdir(_path+'_PNG'):
        ...
        print(czimagepng, end = '\n')
        print(pngimagepath, end = '\n')
        print(outputczpath)
        ...
        if magic=='CZ4' or magic=='CZ3':
            ...
            os.system(rf'image2cz.exe "{czimagepath}" "{pngimagepath}" "{outputczpath}"')
            print(czimagepath,magic ,'ok')
        else: #测试可以全都打成CZ4格式的
            continue
            print(czimagepath,magic )
            os.system(rf'image2cz.exe "C:\InnocentGrey\カルタグラ FHD\files\image\OTHCG\03b" "{pngimagepath}" "{outputczpath}"')
        # if _dir=='OTHCG':
